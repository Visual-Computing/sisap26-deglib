"""
benchmark_task2_spot.py — Run and plot Task 2 spot-check benchmark configurations.

This script runs the 7 benchmark configurations on the llama-small spot-check dataset,
generates a Recall vs Search Time plot, and saves all outputs
(plot, JSON, and Markdown summary) to results/benchmark/task2_spot.
"""
...
def generate_outputs(results: dict[str, Task2Result], output_dir: Path, system_info: dict | None = None) -> None:
    ...
    print(f"[benchmark_task2_spot] Saved detailed JSON to {output_dir / 'results.json'}")
    ...
    plt.title("Task 2 Spot Benchmark — Recall @ 30 vs Search Time")
    ...
    plt.close()
    print(f"[benchmark_task2_spot] Saved plot to {plot_path}")

    # 3. Print & Save Markdown Summary
    md_content = []
    md_content.append("# Task 2 Spot Benchmark Summary — Llama Small dataset")
    ...
    with open(output_dir / "results.md", "w") as f:
        f.write(md_output)
    print(f"[benchmark_task2_spot] Saved markdown summary to {output_dir / 'results.md'}")
...
def main() -> None:
    ...
    output_dir = Path(__file__).parent / "results" / "benchmark" / "task2_spot"
