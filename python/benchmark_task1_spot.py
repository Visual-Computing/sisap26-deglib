"""
benchmark_task1_spot.py — Run and plot Task 1 spot-check benchmark configurations.

Runs all 6 active modes sequentially on the gooaq-small spot-check dataset and prints
a Markdown table so results can be compared.

Usage
-----
    uv run python benchmark_task1_spot.py
...
def print_table(results: dict[str, Task1Result]) -> None:
    print("\n" + "=" * 60)
    print("  Benchmark Results — Spot Dataset (gooaq-small)")
    print("=" * 60)
...
def generate_outputs(results: dict[str, Task1Result], output_dir: Path, system_info: dict | None = None) -> None:
    ...
    print(f"[benchmark_task1_spot] Saved detailed JSON to {output_dir / 'results.json'}")
    ...
    if has_points:
        ...
        plt.title("Task 1 Spot Benchmark — Recall vs Total Time")
        ...
        plt.close()
        print(f"[benchmark_task1_spot] Saved plot to {plot_path}")

    # 3. Print & Save Markdown Summary
    md_content = []
    md_content.append("# Task 1 Spot Benchmark Summary — Spot Dataset (gooaq-small)")
    ...
    with open(output_dir / "results.md", "w") as f:
        f.write(md_output)
    print(f"[benchmark_task1_spot] Saved markdown summary to {output_dir / 'results.md'}")
...
def main() -> None:
    ...
    output_dir = Path(__file__).parent / "results" / "benchmark" / "task1_spot"
