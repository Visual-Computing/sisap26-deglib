...
static ExplorationTimings run_exploration(const deglib::search::SearchGraph& graph,
    ...
    deglib::concurrent::parallel_for(static_cast<size_t>(0), num_chunks, threads, 1, [&](size_t chunk_id, size_t) {
        ...
        for (size_t i = 0; i < num_items; ++i) {
            ...
            deglib::search::ResultSet result_queue = graph.explore(static_cast<uint32_t>(entry_idx), k_search, false, max_distance_count);
        ...
        for (size_t i = 0; i < num_items; ++i) {
            ...
            std::partial_sort(
                candidates.begin(), candidates.begin() + n, candidates.begin() + num_cands, [](const Candidate& a, const Candidate& b) {
...
static int run(const std::filesystem::path& data_path,
    ...
    for (uint32_t evpK_val : evpK_list) {
        for (uint32_t max_dist_val : max_dist_list) {
            ...
            if (!compute_recall && !output_path.empty()) {
                point_output = output_path + "/op_evpK" + std::to_string(evpK_val) + "_md" + std::to_string(max_dist_val) + ".bin";
            }
            auto timings = run_exploration(graph,
                                           k_search,
                                           max_dist_val,
    ...
    sisap_common::print_summary("EVP Build, EVP Explore, FP16 Rerank",
                                4,
                                load_ms,
                                quantize_ms,
                                build_ms,
