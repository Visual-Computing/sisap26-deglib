...
static int run(const std::filesystem::path& data_path,
    ...
    if (!graph_path.empty() && std::filesystem::exists(graph_path)) {
        ...
        if (fs.metric() == deglib::Metric::FP16InnerProduct && fs.dim() == dims && g.size() == count) {
            ...
            std::fprintf(
                ...
                (int)fs.metric(),
                (int)deglib::Metric::FP16InnerProduct,
                (unsigned)fs.dim(),
                dims,
                g.size(),
                count);
    ...
    double build_time_s = (total_load_ms + graph_load_ms + total_build_ms + prune_ms) / 1000.0;
    ...
    for (uint32_t max_dist_val : max_dist_list) {
        ...
        auto timings =
            run_exploration(graph, k_top, max_dist_val, static_cast<uint8_t>(threads), compute_recall, gt_data, point_output, build_time_s);
    ...
    sisap_common::print_summary("FP16 Build, FP16 Explore",
                                1,
                                total_load_ms,
                                0.0,
                                total_build_ms,
