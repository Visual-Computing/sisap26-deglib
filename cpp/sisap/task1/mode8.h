...
/**
 * @file mode8.h
 * @brief Benchmark Mode 8: EVP Asymmetric Linear Search (evp-asymmetric-linear)
 *
 * Results
 * ============================================================
 * Dataset: benchmark-dev-wikipedia-bge-m3-small.h5
 * Recall@15: 78.54 %
 * Explore Time: 942.9 s
 * ============================================================
 *
 * Behavior:
 ...
 * 3. Uses asymmetric distance comparison via FP16EvpAsymmetricSimilarity::compare.
 * 4. Acts as an exact asymmetric similarity search baseline (no graph built).
 */
...
static int run(const std::filesystem::path& data_path,
    ...
    if (train_info.element_size == 4) {
        for (auto& vec : train_vectors) {
            ...
            deglib::distances::floats_to_fp16(
                reinterpret_cast<const float*>(vec.data()), reinterpret_cast<uint16_t*>(fp16_vec.data()), dims);
    ...
    std::printf("=== EVP Asymmetric Linear Search - Mode 8 ===\n");
    ...
    sisap_common::print_summary("EVP Asymmetric Linear Search",
                                8,
                                load_ms,
                                quantize_ms,
                                0.0,
