...
/**
 * @file mode10.h
 * @brief Task 2 Mode 10: FP32 Build (L2-converted) + FP16 Inner Product Search (using InnerProduct Build Metric)
 *
 *  .\deglib_sisap.exe task2  "C:\Data\ANN\sisap2026\llama-dev\llama-dev.h5" mode10 --eps-ext 0.001 --k-ext 64 --k-graph 32 --build-threads 1
 * --max-dist 5000,6000,7000,8000 --eps-search 0.15,0.18,0.19,0.2,0.3 --num-runs 10 --flas
 *
 * Behavior:
 ...
 * 4. Builds a SizeBoundedGraph using Metric::InnerProduct with FloatSpace(dims + 1, Metric::InnerProduct).
 ...
 *    mapping the graph structure built in InnerProduct-space.
...
namespace task2::mode_l2_fp16_ip_build_ip {
...
static int run(const std::filesystem::path& data_path,
    ...
    std::printf("=== L2-transformed IP FP32 Build, FP16 IP Search - Task 2 Mode 10 (opt_target=%s) ===\n",
    ...
    // --------------------------------------------------------------------------
    // Build/Load graph with Metric::InnerProduct (dimension d+1)
    // --------------------------------------------------------------------------
    deglib::FloatSpace feature_space(static_cast<uint32_t>(new_dims), deglib::Metric::InnerProduct);
    ...
    std::printf(
        "Loaded %zu queries (%.2f ms) and converted to FP16 (%.2f ms, dims=%zu)\n", queries.size(), query_load_ms, query_convert_ms, dims);
    ...
    sisap_common::print_summary((use_flas ? "L2-transformed IP Build, FP16 IP Search (FLAS)" : "L2-transformed IP Build, FP16 IP Search"),
                                10,
                                load_ms,
                                transform_ms,
                                build_ms,
