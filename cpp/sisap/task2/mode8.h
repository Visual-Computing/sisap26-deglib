...
/**
 * @file mode8.h
 * @brief Task 2 Mode 8: EVP Linear Search (evp-linear-search)
 *
 * Behavior:
 * 1. Loads FP32 training vectors from "train" as the database.
 * 2. Loads FP32 query vectors from "queries".
 * 3. Quantizes both train and query vectors to EVP bits representation.
 * 4. Runs an exact brute-force linear search over all quantized database vectors for each query.
 * 5. Uses distance comparison via EvpBitsSimilarity::compare.
 * 6. Tracks load, quantization, and search time separately.
 */
...
namespace task2::mode_evp_linear {
...
static ExplorationTimings run_search(
    ...
    const std::vector<std::byte>& quantized_queries,
    size_t db_count,
    size_t query_count,
    size_t dims,
    uint32_t k_top,
        ...
        deglib::concurrent::parallel_for(static_cast<size_t>(0), num_chunks, static_cast<uint32_t>(threads), 1,
            [&](size_t chunk_id, size_t) {
                ...
                for (size_t i = 0; i < num_items; ++i) {
                    ...
                    const std::byte* query_ptr = quantized_queries.data() + q_idx * bytes_per_evp;
    ...
    const std::string& output_path = "")
{
    ...
    std::printf("=== EVP Linear Search (Symmetric) - Task 2 Mode 8 ===\n");
    ...
    std::printf("Loaded %zu queries (dims=%zu)\n", queries_fp32.size(), dims);

    // --------------------------------------------------------------------------
    // Quantize database and query vectors
    // --------------------------------------------------------------------------
    if (non_zeros >= dims) {
    ...
    std::printf("Quantizing database and queries to EVP (non_zeros = %u)...\n", non_zeros);
    ...
    std::vector<float> flat_queries(query_count * dims);
    for (size_t i = 0; i < query_count; ++i) {
        std::memcpy(&flat_queries[i * dims], queries_fp32[i].data(), dims * sizeof(float));
    }
    auto quantized_queries = deglib::quantization::quantize_batch(flat_queries.data(), query_count, static_cast<uint32_t>(dims), non_zeros, threads);
    ...
    // Exploration
    // --------------------------------------------------------------------------
    std::printf("Starting linear search: k_top=%u, threads=%u\n", k_top, threads);
    ...
    auto timings = run_search(quantized_db, quantized_queries, count, query_count, dims, k_top,
                              static_cast<uint8_t>(threads), compute_recall, num_runs, gt_data, point_output, build_time_s);

    std::printf("Linear Search completed: recall %.2f %% and search time %.1f ms\n", timings.recall * 100.0f, timings.search_ms);
