...
static ExplorationTimings run_search(const deglib::graph::SizeBoundedGraph& graph,
...
static int run(const std::filesystem::path& data_path,
    ...
    std::printf("=== FP32 IP Build, FP32 IP Search - Task 2 Baseline (opt_target=%s) ===\n", sisap_common::opt_target_str(opt_target));
    ...
    if (!query_info_ptr) {
        std::fprintf(stderr,
                     "Error: No query dataset found in HDF5 file. Task 2 requires actual test queries (e.g. itest/queries, otest/queries, "
                     "test/queries, or queries).\n");
        return 1;
    }
    ...
    sisap_common::print_summary((use_flas ? "FP32 Build, FP32 Search (FLAS)" : "FP32 Build, FP32 Search"),
                                1,
                                load_ms,
                                0.0,
                                build_ms,
