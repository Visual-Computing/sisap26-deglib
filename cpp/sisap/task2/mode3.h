...
static int run(const std::filesystem::path& data_path,
    ...
    std::printf("=== FP32 IP Build, FP16 IP Search - Task 2 Mode 3 (opt_target=%s) ===\n", sisap_common::opt_target_str(opt_target));
    ...
    double build_time_s = (load_ms + graph_load_ms + build_ms + convert_ms + prune_ms + flas_ms + opt_ms) / 1000.0;
    ...
    sisap_common::print_summary((use_flas ? "FP32 Build, FP16 Search (FLAS)" : "FP32 Build, FP16 Search"),
                                3,
                                load_ms,
                                0.0,
                                build_ms,
